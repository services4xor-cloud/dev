...
function scoreLanguage(me: DimensionProfile, them: DimensionProfile): { score: number; highlight: string | null } {
...
function scoreMarket(them: DimensionProfile, signals: MarketSignal[]): { score: number; highlight: string | null } {
...
function scoreCraft(me: DimensionProfile, them: DimensionProfile): { score: number; highlight: string | null } {
  ...
  if (complementary.length > 0) parts.push(`${complementary.length} complementary craft${complementary.length > 1 ? 's' : ''}`)
  return { score, highlight: `Craft match: ${parts.join(', ')}` }
}

function scorePassion(me: DimensionProfile, them: DimensionProfile): { score: number; highlight: string | null } {
  const shared = intersect(me.interests, them.interests)
  if (shared.length === 0) return { score: 0, highlight: null }
  if (shared.length === 1) return { score: 5, highlight: `Shared interest: ${shared[0]}` }
  if (shared.length === 2) return { score: 10, highlight: `${shared.length} shared interests: ${shared.join(', ')}` }
  return { score: 15, highlight: `${shared.length} shared interests: ${shared.slice(0, 3).join(', ')}${shared.length > 3 ? '...' : ''}` }
}

function scoreLocation(me: DimensionProfile, them: DimensionProfile): { score: number; highlight: string | null } {
...
function scoreFaith(me: DimensionProfile, them: DimensionProfile): { score: number; highlight: string | null } {
...
function scoreReach(me: DimensionProfile, them: DimensionProfile): { score: number; highlight: string | null } {
  let pairScore = 0
...
function scoreCulture(me: DimensionProfile, them: DimensionProfile): { score: number; highlight: string | null } {
