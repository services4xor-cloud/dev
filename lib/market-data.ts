...
const REGION_MAP: Record<string, string[]> = {
  ...
  'Europe': ['DE', 'GB', 'FR', 'CH', 'NL', 'SE', 'NO', 'DK', 'FI', 'IT', 'ES', 'PT', 'AT', 'BE', 'IE'],
...
export const MARKET_SIGNALS: MarketSignal[] = [
  ...
  },
  {
    ...
    description:
      'Lagos and Accra fashion weeks attract international buyers and media attention.',
  ...
  },
  {
    ...
    description:
      'International hotel chains are expanding in Nairobi, Dar es Salaam, and Kigali.',
...
export function getSignalsForCraft(crafts: string[]): MarketSignal[] {
  ...
  return MARKET_SIGNALS.filter((s) =>
    s.demandCrafts.some((dc) => {
      ...
      return lowerCrafts.some(
        (lc) => lowerDc.includes(lc) || lc.includes(lowerDc)
      )
    })
  )
}
