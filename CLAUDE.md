...
| **`REQUIREMENTS.md`** | User requirements, data rules, duplicate inventory | Before refactoring |
...
4. **Mock data** in all pages until `DATABASE_URL` is set (see `lib/db.ts`)
...
NEVER use:       #FF6B35   → eliminated session 6
```
...
| Testing | Jest + Playwright visual tests |
...
├── offerings/                # Kenya offerings: safaris, eco, trade ← NEXT TO BUILD
├── charity/                  # UTAMADUNI CBO
├── about, pricing, contact,  # Info pages
   business, login, signup,   # ← Still need dark-theme conversion
   profile, referral, privacy
└── api/                      # API routes (paths, compass, mpesa, auth…)

lib/
├── vocabulary.ts             # BeNetwork terms — single source of truth
├── country-selector.ts       # Geographic data + proximity engine
...
components/                   # Nav, Footer, PathCard, CountryPrioritySelector…
tests/visual/                 # Playwright smoke + brand + responsive tests
...
npx playwright test     # Visual smoke + brand + responsive tests
...
*Last updated: Session 6 (2026-03-10) · Next: Session 7*
