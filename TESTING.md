...
> **Status: All tests passing (25 Jest + 89 Playwright)**
...
Layer 2: Playwright visual    → every page renders, brand is correct, responsive (89/89 ✅)
```
...
```bash
npx playwright test                        # all visual tests (89/89)
npx playwright test tests/visual/smoke     # smoke only (15/15)
npx playwright test tests/visual/brand     # brand only (26/26)
npx playwright test tests/visual/responsive # responsive only (48/48)
...
#### `tests/visual/smoke.spec.ts` — All pages load, no console errors (15/15 ✅)

Checks every route returns HTTP 200 and has no red console errors.
Viewports: desktop (1280x800) only — fast CI gate.

#### `tests/visual/brand.spec.ts` — Design system compliance (26/26 ✅)
...
#### `tests/visual/responsive.spec.ts` — Multi-breakpoint no-overflow (48/48 ✅)
