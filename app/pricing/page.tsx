    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Link href="/jobs" className="text-gray-600 hover:text-brand-orange">Browse Jobs</Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-orange-50 text-brand-orange px-4 py-2 rounded-full text-sm font-medium mb-4">
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${currency === 'KES' ? 'bg-brand-orange text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-gray-300'}`}
            >
              🇰🇪 KES
            </button>
            <button
              onClick={() => setCurrency('USD')}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${currency === 'USD' ? 'bg-brand-orange text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-gray-300'}`}
            >
              🇺🇸 USD
            </button>
          </div>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl p-6 shadow-sm border-2 transition-transform hover:-translate-y-1 relative ${
                  plan.popular ? 'border-brand-orange shadow-lg' : 'border-gray-100'
                }`}
              >
                {plan.popular && (
                ...
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 ${
                  ...
                  <Icon className={`w-6 h-6 ${plan.popular ? 'text-brand-orange' : 'text-gray-500'}`} />
                </div>
                ...
                <ul className="space-y-2 mb-6">
                  {plan.features.map((f, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
                      <Check className={`w-4 h-4 mt-0.5 flex-shrink-0 ${plan.popular ? 'text-brand-orange' : 'text-green-500'}`} />
                      {f}
                    </li>
                  ))}
                </ul>
        ...
        {/* For Job Seekers */}
        <div className="bg-gradient-to-r from-brand-orange to-orange-500 rounded-2xl p-8 text-white text-center">
          ...
          <Link href="/signup?role=jobseeker" className="bg-white text-brand-orange font-bold px-6 py-3 rounded-xl hover:bg-orange-50 transition-colors inline-block">
            Create Free Profile →
          </Link>
        </div>
      </div>
