    ...
    <div className="min-h-screen bg-[#0A0A0F]">
      ...
      <div className="bg-[#0d0208] border-b border-[#5C0A14]/50">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Link href="/ventures" className="text-gray-400 hover:text-[#C9A227] text-sm">Browse Paths</Link>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-16 space-y-16">
        {/* Referral link */}
        <div className="bg-gray-900/60 rounded-2xl p-8 shadow-sm border border-[#5C0A14]/30 text-center">
          <h2 className="text-2xl font-bold text-white mb-2">Your referral link</h2>
          <p className="text-gray-400 mb-6">Share this link — every signup counts toward your earnings</p>

          <div className="flex items-center gap-3 bg-[#0A0A0F] rounded-xl p-4 border border-[#5C0A14]/30 mb-4">
            <span className="flex-1 text-sm text-gray-300 font-mono truncate">{mockLink}</span>
            <button
              onClick={copy}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                copied ? 'bg-green-900/40 text-green-400' : 'bg-[#5C0A14] text-white hover:bg-[#5C0A14]/80'
              }`}
            >
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {steps.map((step, i) => (
              <div key={step.n} className="relative">
                <div className="bg-gray-900/60 rounded-2xl p-6 shadow-sm border border-[#5C0A14]/30 h-full">
                  <div className="w-10 h-10 bg-[#5C0A14] rounded-full flex items-center justify-center text-white font-bold mb-4">
                    {step.n}
                  </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          {[
            ...
            return (
              <div key={stat.label} className="bg-gray-900/60 rounded-2xl p-6 text-center shadow-sm border border-[#5C0A14]/30">
